...
fun Route.adminModule(manager: DeploymentManager) {
    get("/") {
            ...
            body {
                ...
                div {
                    ...
                    table {
                        ...
                        for (assignment in manager.getAssignments().sortedBy { it.workspace.hash().hash }.sortedBy { it.workspace.id }) {
    ...
    post("disable-instance") {
        ...
        manager.disableInstance(instanceId)
        call.respondRedirect(".")
    }

    post("enable-instance") {
        val instanceId = InstanceName(call.receiveParameters()["instanceId"]!!)
        manager.enableInstance(instanceId)
        call.respondRedirect(".")
    }
    ...
    post("change-unassigned") {
        ...
        manager.changeNumberOfAssigned(WorkspaceHash(workspaceHash), numberOfUnassigned.toInt().coerceIn(0..100))
        call.respondRedirect(".")
    }

    get("log/{instanceId}/content") {
        val instanceId = InstanceName(call.parameters["instanceId"]!!)
        val log = manager.getPodLogs(instanceId) ?: "Instance $instanceId not found"
    ...
    get("log/{instanceId}/events") {
        ...
        val events = manager.getEvents(instanceId)
